import { defaultObj, intersection, pick } from '@keystone-next/utils-legacy';
...
const checkSchemaNames = ({
  ...
  const keyedBySchemaName = typeof access === 'object' && intersection(Object.keys(access), schemaNames).length === Object.keys(access).length;
...
function parseCustomAccess({
  ...
  const fullAccess = keyedBySchemaName ? _objectSpread2(_objectSpread2({}, defaultObj(schemaNames, defaultAccess)), access) // Access keyed by schemaName
...
function parseListAccess({
  ...
  const parseAndValidate = (access = {}) => {
    if (typeof access === 'boolean' || typeof access === 'function') {
      ...
      return _objectSpread2(_objectSpread2({}, defaultObj(accessTypes, defaultAccess)), pick(_access, accessTypes));
    } else {
...
export { parseCustomAccess, parseFieldAccess, parseListAccess, validateAuthAccessControl, validateCustomAccessControl, validateFieldAccessControl, validateListAccessControl };
