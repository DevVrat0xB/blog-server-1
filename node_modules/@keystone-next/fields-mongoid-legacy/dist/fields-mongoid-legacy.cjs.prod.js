...
class MongoIdImplementation extends fieldsLegacy.Implementation {
  get _supportsUnique() {
    return true;
  }
...
class MongooseMongoIdInterface extends adapterMongooseLegacy.MongooseFieldAdapter {
...
class KnexMongoIdInterface extends adapterKnexLegacy.KnexFieldAdapter {
  constructor() {
    super(...arguments);
...
class PrismaMongoIdInterface extends adapterPrismaLegacy.PrismaFieldAdapter {
  constructor() {
    super(...arguments);
    this.isUnique = !!this.config.isUnique;
    this.isIndexed = !!this.config.isIndexed && !this.config.isUnique;
...
const pkgDir = path__default['default'].dirname(require.resolve('@keystone-next/fields-mongoid-legacy/package.json'));
const MongoId = {
  type: 'MongoId',
  implementation: MongoIdImplementation,
  views: {
    Controller: path__default['default'].join(pkgDir, 'views/Controller'),
    Field: fieldsLegacy.Text.views.Field,
  ...
  primaryKeyDefaults: {
    knex: {
      getConfig: () => {
        throw `The MongoId field type doesn't provide a default primary key field configuration for knex. ` + `You'll need to supply your own 'id' field for each list or use a different field type for your ` + `ids (eg '@keystone-next/fields-auto-increment-legacy').`;
      }
    },
    prisma: {
      getConfig: () => {
        throw `The MongoId field type doesn't provide a default primary key field configuration for Prisma. ` + `You'll need to supply your own 'id' field for each list or use a different field type for your ` + `ids (eg '@keystone-next/fields-auto-increment-legacy').`;
      }
    },
    mongoose: {
      getConfig: () => ({
